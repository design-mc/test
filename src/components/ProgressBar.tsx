...
const ProgressBar: React.FC<ProgressBarProps> = ({ completedTiles, totalTiles }) => {
  const surveyTiles = totalTiles / 2; // Assuming half the tiles are surveys
  const percentage = (completedTiles / surveyTiles) * 100;
  
  return (
    <div className="mb-8">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-lg font-semibold text-gray-800">Your Progress</h3>
        <span className="text-indigo-600 font-medium">{completedTiles} of {surveyTiles} completed</span>
      </div>
